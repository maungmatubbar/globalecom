    ...
    @if (isset($page_name) && $page_name=='listing' && !isset($_REQUEST['search']))
        <div class="well well-small">
            ...
            @foreach ($fabricArray as $fabric)
                <input class="fabric" style="margin-top:-3px;" type="checkbox" name="faric[]" id={{ $fabric }} value="{{ $fabric }}"> {{ $fabric }} <br>
            @endforeach
        </div>
        <div class="well well-small">
            <h5>Sleeve</h5>
        ...
        <div class="well well-small">
            ...
            @foreach ($patternArray as $pattern)
                <input class="pattern" style="margin-top:-3px;" type="checkbox" name="faric[]" id={{ $pattern }} value="{{ $pattern }}"> {{ $pattern }} <br>
            @endforeach
        </div>
        <div class="well well-small">
            <h5>Fit</h5>
